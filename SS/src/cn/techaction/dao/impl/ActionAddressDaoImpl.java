...
public class ActionAddressDaoImpl implements ActionAddressDao {
	...
	public ActionAddressVo findAddressById(Integer id) {
		...
		try {
			return queryRunner.query(sql, new BeanHandler<ActionAddressVo>(ActionAddressVo.class),id);
	...
	public ActionAddress findAddrsByIdP(Integer addrId) {
		String sql = "select id,user_id as uid,name,phone,mobile,"
				+ "province,city,district,addr,zip,default_addr,"
				+ "del_state,created,updated from action_address where id = ? and del_state=0";
	...
	public List<ActionAddress> findAddrsByUserIdP(Integer userId) {
		String sql = "select id,user_id as uid,name,phone,mobile,"
				+ "province,city,district,addr,zip,default_addr,"
				+ "del_state,created,updated from action_address"
				+ " where user_id = ? and del_state=0 order by default_addr desc,updated desc ";	
		try {
	...
	public ActionAddress findDefaultAddrP(Integer userId) {
		String sql = "select id,user_id as uid,name,phone,"
				+ "mobile,province,city,district,addr,zip,default_addr,"
				+ "del_state,created,updated from action_address where user_id = ? and default_addr = 1 and del_state = 0";
		try {
	...
	public List<ActionAddress> findAddressByIdP(Integer id) {
		String sql ="select id,user_id as uid,name,phone,mobile,"
				+ "province,city,district,addr,zip,default_addr,"
				+ "del_state,created,updated from action_address"
				+ " where id = ? and del_state=0 order by default_addr desc,updated desc ";
		try{
			return queryRunner.query(sql, new BeanListHandler<ActionAddress>(ActionAddress.class), id);
		}catch(SQLException e){
			e.printStackTrace();
			return null;
		}
