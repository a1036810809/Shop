...
public class UserServiceImpl implements UserService {
	...
	public SverResponse<String> doRegisterP(User user) {
		//检查用户名是否存在
		 SverResponse<String> resp = checkValidationP(user.getAccount(),ConstUtil.TYPE_ACCOUNT);
		 if(!resp.isSuccess()) {
			 return resp;
		 }
		//检查邮箱是否被注册
		resp = checkValidationP(user.getEmail(),ConstUtil.TYPE_EMAIL);
		if(!resp.isSuccess()) {
			return resp;
		}
		...
		int rs = userDao.insertUserP(user);
		if(rs==0) {
	...
	public SverResponse<User> findUserByAccountP(String account) {
		//通过用户名查找到用户
		User user = userDao.findUserByAccountP(account);
		if(user==null) {
		...
		if(StringUtils.isNotBlank(type)) {
			...
			if(StringUtils.isNotBlank(type)) {
				if(ConstUtil.TYPE_EMAIL.equals(type)) {
					int rs = userDao.checkUserByEmailP(str);
					if(rs > 0) {
			...
			if(StringUtils.isNotBlank(type)) {
				if(ConstUtil.TYPE_PHONE.equals(type)) {
					int rs = userDao.checkUserByPhoneP(str);
					if(rs > 0) {
	...
	public SverResponse<String> checkUserAnswerP(String account, String question, String asw) {
		//获取校验结果
		int rs = userDao.checkUserAnswerP(account,question,asw);
		if(rs > 0) {
			//答案正确，生成token
	...
	public SverResponse<String> updatePasswordP(User user, String newPassword, String oldPassword) {
		...
		int rs = userDao.checkPasswordP(user.getAccount(),oldPassword);
		if(rs == 0) {
	...
	public SverResponse<String> getUserQuestionP(String account) {
		User user = userDao.getUserQuestionP(account);
		if(user==null){
