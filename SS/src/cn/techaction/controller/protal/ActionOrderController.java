...
public class ActionOrderController {
	...
	public SverResponse<PageBean<ActionOrderVo>> list(HttpSession session,Integer status,
		...
		return aOrderService.findOrdersP(user.getId(),status,pageNum,pageSize);
	}
	/**
	 * 取消订单
	 * @param session
	...
	public SverResponse<String> cancelOrder(HttpSession session,Long orderNo){
		...
		return aOrderService.cancelOrderP(user.getId(),orderNo);
	}
	...
	public SverResponse<ActionOrderVo> getDetail(HttpSession session,Long orderNo){
		...
		return aOrderService.findOrderDetailP(user.getId(),orderNo);
	}
	/**
	 * 创建订单
	 * @param session
	...
	public SverResponse<ActionOrderVo> createOrder(HttpSession session,Integer addrId){
		...
		return aOrderService.generateOrderP(user.getId(),addrId);
	}
	/**
	 * 付款 
	 * @param session
	...
	@ResponseBody
	public SverResponse<String> payOrder(HttpSession session,Long orderNo){
		//判断用户是否登录
		User user = (User) session.getAttribute(ConstUtil.CUR_USER);
		if(user==null) {
			return SverResponse.createByErrorMessage("请登录后再进行操作！");
		}
		return aOrderService.payOrder(orderNo);
	}
}
